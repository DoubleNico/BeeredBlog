<template>
  <div class="min-h-screen flex bg-gray-50 dark:bg-gray-900">
    ...
    >
      <div
        class="absolute inset-0 bg-gradient-to-br from-primary-900/40 to-gray-900/60"
      />
      <div
        class="absolute -top-24 -left-24 w-96 h-96 bg-white/10 rounded-full blur-3xl"
      />
      <div
        class="absolute -bottom-24 -right-24 w-96 h-96 bg-white/5 rounded-full blur-3xl"
      />

      <div class="relative z-10 max-w-lg text-center">
        <div
          class="w-20 h-20 rounded-2xl bg-white/10 border border-white/20 flex items-center justify-center mx-auto mb-8 shadow-xl backdrop-blur-sm"
        >
          <UIcon
        ...
        <p class="text-white/80 text-lg mb-12 leading-relaxed">
          Sign in to manage your blogs and connect with your readers.
        </p>

        <div class="grid grid-cols-3 gap-6 border-t border-white/10 pt-8">
          <div>
            <div class="text-2xl font-bold text-white mb-1">1K+</div>
            <div class="text-sm text-white/70 font-medium">Writers</div>
          </div>
          <div>
            <div class="text-2xl font-bold text-white mb-1">5K+</div>
            <div class="text-sm text-white/70 font-medium">Articles</div>
          </div>
          <div>
            <div class="text-2xl font-bold text-white mb-1">10K+</div>
            <div class="text-sm text-white/70 font-medium">Readers</div>
          </div>
        </div>
      </div>
    </div>
...
const handleSocialLogin = (provider: 'github' | 'google') => {
  return navigateTo(`/auth/${provider}?state=login`, { external: true })
}

useHead({
  title: 'Sign In - JavaBlog',
})
</script>
